...
@Deprecated('this is travisDartChannel instead!')
String get travisDartVersion =>
    Platform.environment['TRAVIS_DART_VERSION'] ?? 'stable';

String get travisDartChannel =>
    Platform.environment['TRAVIS_DART_VERSION'] ?? 'stable';

/// To deprecate
@Deprecated('use ciFlutterTop')
String get travisFlutterTop => ciFlutterTop;

String get ciFlutterTop =>
    '$_homePath/.tekartik/travis/$travisDartChannel/flutter';
...
export PATH=$ciFlutterTop/bin:$ciFlutterTop/bin/cache/dart-sdk/bin:\$PATH
...
/// Flutter install params
class CiFlutterSetup {
  final String channel;
  final String directory;

  CiFlutterSetup({required this.channel, required this.directory});

  /// Get the environment
  ShellEnvironment getEnvironment() {
    var shellEnvironment = ShellEnvironment();
    shellEnvironment.paths.insertAll(0, [
      join(directory, 'bin'),
      join('directory', 'bin', 'cache', 'dart-sdk', 'bin')
    ]);
    return shellEnvironment;
  }

  Future<void> upgrade({bool force = false}) async {
    var shellEnvironment = getEnvironment();
    var shell = Shell(environment: shellEnvironment);
    await shell.run('flutter upgrade${force ? ' --force' : ''}');
  }

  /// Install flutter in the specified directory.
  Future install({bool? force}) async {
    force ??= false;
    var shell = Shell();
    ...
    await Directory(dirname(directory)).create(recursive: true);
...
///
/// Compat - to deprecate
// @Deprecated('User CiFlutterSetup')
Future install(
        {required String channel, required String directory, bool? force}) =>
    CiFlutterSetup(channel: channel, directory: directory)
        .install(force: force);
...
//  'PAGER': 'cat',
//  'RACK_ENV': 'test',
//  'TRAVIS_CMD': 'dart tool/install.dart',
